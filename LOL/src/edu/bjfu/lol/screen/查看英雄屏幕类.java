...
public class 查看英雄屏幕类 extends 屏幕类
{
	private static final String 查看英雄屏幕目录 = 常量类.图片目录 + "ViewHeroScreen/";
	private static final String 图片打包文件名 = 查看英雄屏幕目录 + "Pack.txt";
	private 自适应舞台类 舞台;
	private Skin 皮肤资源;
	...
	private class 英雄信息条演员类 extends Group
	{
		...
		public 英雄信息条演员类(英雄类 英雄)
		{
			...
			Image 头像图片 = new Image(英雄.get英雄头像());
			头像图片.setPosition(英雄列表演员类.左右间距, (getHeight() - 头像图片.getHeight()) / 2);
			Label 名字标签 = new Label(英雄.get英雄名字(), 名字标签格式);
			...
			addListener(new InputListener()
			{
				...
				public void touchUp(InputEvent event, float x, float y, int pointer, int button)
				{
					...
					if (!is拖动了)
					{
						背景图片.setDrawable(背景弹起);
						主类.实例.播放按钮声音();
						主类.实例.进入屏幕(new 英雄详细屏幕类(英雄信息条演员类.this.英雄));
					}
				}
				@Override
